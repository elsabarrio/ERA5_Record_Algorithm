...
if(!dir.exists(outdir)){
  ...
  length(tab.years)
  idx.train <- which(!is.na(match(idx.years,names(tab.years)[2:51])))
  ...
  for(ss in 1:nrow(stations)){
    ...
    predict.test <- predict(best_model,
                            newdata = bin_df[idx.test,],
                            type = "response")
    roc.aux <- roc(bin_df[idx.test,]$Ix, predict.test, print.auc = TRUE)
    print(auc(roc.aux))
    ...
    # Save AUC matrix
    write.csv(x = AUC.df,
              file = file.path(dirname(outdir),"local_AUC.csv"),
              row.names = F)
    toc <- Sys.time()
