...
Constructing a global model with interaction of the final variables selected 
from the only geopotential variables (M1) with the interaction of latitude plus
longitude.
...
Constructing a global model with interaction of the final variables selected 
from the only geopotential variables (M1) with the interaction of average and SD of Tx for the reference
period (1981-2010).
...
## M4: M1 * (altitude + log(oast distance))

Constructing a global model with interaction of the final variables selected 
from the only geopotential variables (M1) with the interaction of altitude and 
the natural logarithm of distance to the coast.
...
Constructing a global model with interaction of the final variables selected 
from the only geopotential variables (M1) with the interaction of geodesic
(latitude and longitude), climatic (average and SD Tx), and geographical (altitude and log(CoastDist)) variables.
...
if(!file.exists(model_file)) {
  ...
  cat("M5: ")
  print(toc-tic)
  # Time difference of 41.21845 mins
