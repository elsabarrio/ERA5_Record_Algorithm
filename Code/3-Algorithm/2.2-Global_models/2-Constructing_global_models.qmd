...
To construct the final models, we first need to construct a data frame containing the data for all stations. To do so, we perform \*\*column-wise concatenation\*\*. Specifically, we combine data from 36 separate data frames, each corresponding to a different station, into a single larger data frame. Since these data frames share the same variables, the operation involves appending the columns of each station's data frame side by side. The variables corresponding to the closest point to each of the stations are not the same, but we are gonna merge them into a unique variable.

```{r}
if(!is.element("OptimalCutpoints", row.names(installed.packages()))) install.packages("OptimalCutpoints")
...
tx <- read.csv(file = file.path(data_dir,'tx.data/Tx_mat.csv'))
...
# Data directory
outdir <- paste0(wd,"/Results/final.models")
...
# Add average temperature and standard deviation to stations df for a period of interest
