...
data.dir <- paste0(wd,"/Results/local.z.values/")

# Read the z values
z.mat <- read.csv(file = file.path(data.dir, "z.values.df.csv"), check.names = FALSE)
...
for (original_col in originals) {
  # Find the equivalen first polynomial term column
  poly_col <- poly.2.1[grepl(paste0("\\(", original_col, ","), poly.2.1)]
  
  # Realizar la suma en valor absoluto
  z.mat[[poly_col]] <- rowSums(cbind(abs(z.mat[[original_col]]), abs(z.mat[[poly_col]])), na.rm = TRUE)
}
...
folder_path <- "C:/Users/elsac/Documents/Excess.records/Results/final.models"
...
saveRDS(final.var.aux,file = paste0(folder_path,"/final.variables.rds"))
```
