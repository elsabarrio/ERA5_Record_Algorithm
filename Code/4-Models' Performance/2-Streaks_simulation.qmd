...
results <- array(list(), dim = c(64, 36))

for (year in 1:64) {
  for (station in 1:36) {
    series <- itx3d[year, , station]
    results[[year, station]] <- count_ones_runs_with_days(series)
  }
}

colnames(results) <- stations$STAID

# View an example of the results for the first year and the first station
print(results[[64,1]])

#saveRDS(file = "Results/Streaks/streaks.counts.RDS", results)
```
...
```{r}
# Read models predict
predict.df <- read.csv2(file.path(wd,"Results/M2.predict.csv"))

# Define number of stations
lon.stat <- 36

# Read the real streaks
res.true <- results

# Create empty matrix to fill with the true probabilities
true.probs <- matrix(0, nrow = lon.stat, ncol = 5)

# Create a loop to obtain the probabilities of different streaks lengths
# per station for all the test period
for (stat in 1:ncol(res.true)){
  aux.streaks <- vector('numeric')
  for(year in min(predict.df$t):max(predict.df$t)){
    aux.streaks <- c(aux.streaks, res.true[[year,stat]]$lengths)
  }
  
  table.prob <- table(aux.streaks)/sum(table(aux.streaks))
  
  idx.num <- which(names(table.prob) == 1)
  if (length(idx.num) > 0) true.probs[stat,1] <- table.prob[idx.num]
  
  idx.num <- which(names(table.prob) == 2)
  if (length(idx.num) > 0) true.probs[stat,2] <- table.prob[idx.num]
  
  idx.num <- which(names(table.prob) == 3)
  if (length(idx.num) > 0) true.probs[stat,3] <- table.prob[idx.num]
  
  idx.num <- which(names(table.prob) == 4)
  if (length(idx.num) > 0) true.probs[stat,4] <- table.prob[idx.num]
  
  idx.num <- which(names(table.prob) >= 5)
  if (length(idx.num) > 0) true.probs[stat,5] <- sum(table.prob[idx.num])
...
for (i in 1:nrow(mat.true.errors)) mat.true.errors[i,] <- quantile(true.probs[,i], probs = c(0.025,0.975))

# Now add the true probabilities to the data frame
true.df <- as.data.frame(mat.true.errors)
true.df$true.probs <- apply(true.probs, 2, mean)
true.df$value <- 1:5
...
lon.all <- length(predict.df$predict.test)
