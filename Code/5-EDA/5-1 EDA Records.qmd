...
# Smoothed plot
g3d <- ggplot(data = trnd_df,
              mapping =  aes(x=t)) +
  geom_hline(yintercept=0, color = "gray") +
  #Tx
  geom_smooth(aes(y=ztx), color="black", se = F) +
  #G300
  geom_smooth(aes(y=G300), color="blue", se = F) +
  #G500
  geom_smooth(aes(y=G500), color="green", se = F) +
  #G700
  geom_smooth(aes(y=G700), color="red", se = F) +
  ylab("z") +
  xlab("t (year)") +
  scale_x_continuous(breaks = c(1, 21, 41, 61),
                     labels = c("1 (1960)", "21 (1980)", "41 (2000)", " 61 (2020)")) +
...
# Save
ggsave(filename = "trend_z_Ix_G_loess.pdf",
       plot = g3d,
       device = "pdf",
       path = out_dir,
       width = 5,
