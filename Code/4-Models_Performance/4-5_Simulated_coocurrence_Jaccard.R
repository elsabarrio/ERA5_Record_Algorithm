...
ji_obs_df$JIrdm <- apply(ji_rdm_mat,1,mean)
# Plot it
g2 <- ggplot(data = ji_obs_df,
             mapping =  aes(x=dist)) +
  geom_smooth(mapping = aes(y=JI), se = T) +
  geom_smooth(mapping = aes(y=JIrdm), col = "red", se = T) +
  theme_bw() +
  labs(title = "Observed pairwise Jaccard Index",
       subtitle = "2011-2023") +
  ylab('Jaccard\'s index') +
...
# Calculate simulated JI under M2 predictions
ji_m2_mat <- matrix(0, nrow = nrow(ji_obs_df), ncol = nperm)
# On y va
for(pp in 1:nperm){
  if(pp %% 100 == 0) cat(paste0("..",pp))
...
# Plot it
g3 <- ggplot(data = ji_obs_df,
             mapping =  aes(x=dist)) +
  geom_smooth(mapping = aes(y=JI), se = T) +
  geom_smooth(mapping = aes(y=JIrdm), col = "red", se = T) +
  geom_smooth(mapping = aes(y=JIsim), col = "green", se = T) +
  theme_bw() +
  labs(title = "Observed pairwise Jaccard Index",
       subtitle = "2011-2023") +
  ylab('Jaccard\'s index') +
...
cor(ji_obs_df$JI,ji_obs_df$JIsim)
cor(ji_obs_df$JI,ji_obs_df$JIsim, method = "spearman")
g4 <- ggplot(ji_obs_df, aes(x=JI, y=JIsim)) +
  geom_point() +
  geom_smooth(method="lm", col="red") +
  labs(title = "Jaccard's Index",
...
ggsave(filename = "ji_simM2.pdf", plot = g4,
