...
atm_var_station <- function(sta_name, data_dir, functions_dir, altitudes, variable, folder, itx3d, stations, days){
  ...
  for(aa in 1:length(altitudes)) {
    ...
    print(paste0("Level ",altitudes[aa],"hPa"))
    
    # Loop through each pair of latitude and longitude values
    for (gg in 1:nrow(small_grid)) {
      
      # Extract grid point LAT and LON
      glat <- small_grid$latitude[gg]
      glon <- small_grid$longitude[gg]
      print(paste0("Grid point: ",glat,",",glon))
      ...
      var_name <- paste0(variable,".", altitudes[aa], ".", lat_name,".",lon_name)
      bin_df[var_name] <- get_atmospheric_data(folder_path = folder,
                                               ftime = "12pm",
                                               variable =  variable,
    ...
    for (gg in 1:nrow(large_grid)) {
      
      # Extract grid point LAT and LON
      glat <- large_grid$latitude[gg]
      glon <- large_grid$longitude[gg]
      print(paste0("Grid point: ",glat,",",glon))
